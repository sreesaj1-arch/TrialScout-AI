...
A2A_RPC_URL = BASE_URL + "/a2a/trialscout_agent/"
...
def start_server() -> subprocess.Popen[str]:
    ...
    command = [
        ...
        "-m",
        "uvicorn",
        "trialscout_agent.fast_api_app:app",
        "--host",
        "0.0.0.0",
        "--port",
        "8000",
...
def test_adk_run_sse(server_fixture: subprocess.Popen[str]) -> None:
    ...
    session_response = requests.post(
        f"{BASE_URL}/apps/trialscout_agent/users/{user_id}/sessions",
        headers=HEADERS,
        json=session_data,
        timeout=60,
    )
